...
export default function AppSidebar({ user }: { user: User }) {
  ...
  return (
    <aside
      className="fixed left-0 top-0 h-screen t-card border-r t-border flex flex-col z-40 select-none transition-[width] duration-200 overflow-hidden"
      style={{ width: collapsed ? 64 : 220 }}
    >
      {/* Brand + collapse toggle */}
      <div className="px-3 pt-4 pb-3 border-b t-border flex items-center gap-2">
        {!collapsed && (
          <Link href="/dashboard" className="flex-1 min-w-0 pl-2">
        ...
        <button
          onClick={toggle}
          className="shrink-0 w-9 h-9 rounded-lg flex items-center justify-center t-muted hover:t-text hover:t-hover transition-colors"
      ...
      <div className="border-t t-border px-3 py-3 space-y-2">
        ...
        {collapsed && (
          <button
            onClick={handleLogout}
            title="Logout"
            className="w-full flex items-center justify-center py-2 t-muted hover:t-error transition-colors"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
